...
import { AlgoOrderManager } from './AlgoOrderManager';
...
export function MarketDashboard({ initialSymbol }: MarketDashboardProps = {}) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 text-gray-900">
      ...
      {/* Main View Port rendering based on terminalTab */}
      {terminalTab === 'algo' ? (
        <div className="flex-1 p-4 sm:p-6 overflow-hidden">
          <AlgoOrderManager symbol={selectedSymbol} currentPrice={currentPrice} />
        </div>
      ) : (
          ...
          {terminalTab === 'dom' ? (
            <div className="w-full lg:w-96 bg-gray-50 border-t lg:border-t-0 lg:border-l border-gray-200 p-4 shrink-0 lg:overflow-y-auto">
              <OrderBookDOM
                ...
                onSelectPrice={(_p) => {
                  setTradeQuantity((prev) => prev || 0.1);
                  setTerminalTab('standard');
                }}
              />
            </div>
          ) : (
